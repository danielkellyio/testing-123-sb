...
export default ({ types: t, template }: any) => {
  // We set a globalThis so that we can later fail the test
  // even if the error ends up being caught by the code.
  const buildGuard = template(`
    if (!globalThis.allowInfiniteLoops && ITERATOR++ > MAX_ITERATIONS) {
      if (!globalThis.infiniteLoopError) {
        globalThis.infiniteLoopError = new RangeError(
          'Potential infinite loop: exceeded ' +
          MAX_ITERATIONS +
          ' iterations. To disable this error, set "window.allowInfiniteLoops = true".'
        );
      }
      throw globalThis.infiniteLoopError;
    }
  `);
